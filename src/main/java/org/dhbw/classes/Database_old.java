...
//            statement.close();
    ...
    private static int connectToPeron(Person person) throws SQLException, ClassNotFoundException {
        ResultSet rs = null;
        if (person.getBirthday() == null || person.getAddress() == null)
            rs = getFromDatabase("SELECT student_id FROM student LEFT JOIN person ON student.person_id = person.person_id WHERE person.last_name = '" + person.getName() + "' AND person.first_name = '" + person.getForename() + "' AND person.email = '" + person.getEmail() + "'" +
                    " UNION SELECT docent_id FROM docent LEFT JOIN person ON docent.person_id = person.person_id WHERE person.last_name = '" + person.getName() + "' AND person.first_name = '" + person.getForename() + "' AND person.email = '" + person.getEmail() + "'" +
                    " UNION SELECT company_id FROM company LEFT JOIN person ON company.contact_person_id = person.person_id WHERE person.last_name = '" + person.getName() + "' AND person.first_name = '" + person.getForename() + "' AND person.email = '" + person.getEmail() + "'");
        else {
            rs = getFromDatabase("SELECT student_id FROM student LEFT JOIN person ON student.person_id = person.person_id WHERE person.last_name = '" + person.getName() + "' AND person.first_name = '" + person.getForename() + "' AND person.birthdate = '" + (dateFormat.format(person.getBirthday())) + "' AND person.email = '" + person.getEmail() + "' AND person.address_id = " + getAddressID(person.getAddress()) +
    ...
    public static void updateStudent(DualStudent student, int id) throws SQLException, ClassNotFoundException {
        PreparedStatement preparedStatement = getConnection().prepareStatement("UPDATE student SET java_knowlage = ?, person_id = ?, course_id = ?, company_id = ? WHERE student.student_id = ?");
    ...
    public static int updateCompany(Company company, int company_id) throws SQLException, ClassNotFoundException {
        ...
        PreparedStatement preparedStatement = getConnection().prepareStatement("UPDATE company SET company.name = ?, address_id = ?, contact_person_id = ? WHERE company_id = ?");
        preparedStatement.setString(1, company.getName());
        int address_id;
    ...
    private static int updateAddress(Address address, int id) throws SQLException, ClassNotFoundException {
        PreparedStatement preparedStatement = getConnection().prepareStatement("UPDATE address SET street = ? AND address.number = ? AND postal_code = ? AND city = ? AND country = ? WHERE address_id = ?");
        preparedStatement.setString(1, address.getStreet());
        preparedStatement.setString(2, address.getNumber());
        preparedStatement.setString(3, address.getPostcode());
        preparedStatement.setString(4, address.getCity());
